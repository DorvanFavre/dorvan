...
class ServoDriver:
    def __init__(self,verbose=1, offset_file='offsets.json', limits_file='angle_limits.json'):
    ...
    def set_servo_angle(self, leg, servo, angle, is_radian=False):
        """Set the servo angle ind deg with an offset and print it."""
        if is_radian:
        ...
        try:
            ...
            print(f"Could not set the angle: {e}")
    ...
    def calibrate(self):
        ...
        while True:
            ...
            while True:
                action = input("Increase (i), Decrease (d), or 'q' to quit calibration mode: ")
                if action.lower() == 'q':
                    break
                offset_key = self._get_offset_key(leg, servo)
                ...
                if action.lower() == 'i':
                    ...
                    print("Invalid action. Please choose 'i', 'd', or 'q'.")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--idle', action='store_true', help="Idle position")
    parser.add_argument('--ninety', action='store_true', help="90 position")
    ...
    if args.idle:
        for leg in range(6):
                driver.set_servo_angle(leg, 0, 90)
                driver.set_servo_angle(leg, 1, 180)
                driver.set_servo_angle(leg, 2, 90)
    
    if args.ninety:
        for leg in range(6):
                driver.set_servo_angle(leg, 0, 90)
                driver.set_servo_angle(leg, 1, 90)
                driver.set_servo_angle(leg, 2, 90)
